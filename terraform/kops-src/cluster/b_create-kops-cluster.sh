...
kops create cluster server-cluster --zones $GCPZONE --state "gs://$PROJECT-kops-clusters/"/ --project=$PROJECT --node-count=4
#echo "Cluster object has been created:"
echo "Starting cluster instances now"
kops update cluster server-cluster --yes --state "gs://$PROJECT-kops-clusters"/